...
- [x] **צעד 8** — הרצה ידנית מלאה של הצנרת ב-Phase 0 בוצעה 16.8 ריצת ערב, ששת הסוכנים כסאב-איג'נטים נפרדים. **כל סוכן כתב אך ורק את קובץ החוזה שלו — אומת מול `git status`.** תוצאה: P-2026-08-16-02 אושרה ומולאה בנייר (50 USDT BTC @ 63,150), T-2026-08-16-02 (BNB @604) חתום ופעיל, T-2026-08-16-03 נדחה בגלל תקרת 10%, T-2026-08-16-01 בוטל. פתוח מהריצה: ראה "ממצאי צעד 8" למטה
...
## ממצאי צעד 8 (16.8, ריצת ערב)

1. **להכרעת דניאל — תקרת 10% לפי נכס או לפי דלי?** `limits.json` לא מגדיר אם
   `max_position_pct_of_portfolio` נמדדת על הנכס (Core+Satellite יחד) או על כל דלי
   בנפרד. מרים פסקה לפי נכס, בפרשנות המחמירה, וזו הסיבה היחידה ש-T-03 נדחה.
   הכרעה כזו לא צריכה לשבת בשיקול דעת של סוכן. **הקובץ בעריכת דניאל בלבד.**
2. **תוקן — קוד יציאה 1 על הצלחה.** `sign_trigger.py` ו-`kill_check.py` נפלו
   ב-`UnicodeEncodeError` על הדפסה עברית *אחרי* שהפעולה הצליחה. התיקון ב-
   `binance_client.py` (reconfigure ל-UTF-8), אומת: `kill_check.py` מחזיר 0.
3. **פתוח — `sign_trigger.py` חסר מסלול invalidate.** מרים צריכה לבטל טריגר דרך
   הסקריפט, כמו שהיא חותמת דרכו.
4. **פתוח — חותמת פסק דין עתידית.** חותמת מרים הקדימה את שעון המערכת ב-13 דקות.
   שער הטריות בודק רק "לא ישן מדי", ולכן חותמת עתידית שגויה עוברת. להוסיף בדיקה
   ב-`place_order.py`.
5. **תוקן — הערת הסביבה ב-CLAUDE.md.** נקבע שם ש-egress חסום; זה נכון ל-sandbox
   בלבד. מהמחשב של דניאל שלושת ה-hosts מחזירים 200, כולל testnet.
6. **לקח לגדי:** דיווח מילולי של סוכן אינו ראיה. בריצה הזו התקבל דיווח שסתר את
   הקובץ שהסוכן עצמו כתב, ודיווח נוסף שטען לסתירת מחירים בין תמר ליואב שלא
   התקיימה בקבצים. **המקור הקובע הוא הקובץ, תמיד.**
